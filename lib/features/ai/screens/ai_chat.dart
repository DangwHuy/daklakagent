  ...
  void _sendMessage(String text) async {
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('users').doc(user.uid).collection('ai_chat_history').add({
        ...
        'order': timestamp, // Thêm trường này để sắp xếp cứng
      });
      ...
      await FirebaseFirestore.instance.collection('users').doc(user.uid).collection('ai_chat_history').add({
        ...
        'action': actionCode,
        'createdAt': FieldValue.serverTimestamp(),
        'order': timestamp + 1, // Đảm bảo tin nhắn AI luôn sau User
      });
...
// KIỂM TRA BÀN PHÍM CÓ ĐANG MỞ HAY KHÔNG
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          // --- PHẦN 1 & 2: KHÔNG GIAN NỘI DUNG CHÍNH (Đồng bộ qua Stream) ---
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                // --- NẾU CÓ CHAT: Hiển thị đoạn chat (Ẩn màn hình Menu) ---
                if (docs.isNotEmpty) {
                  // Đồng bộ trạng thái ngầm để tắt Title AppBar
                  WidgetsBinding.instance.addPostFrameCallback((_) {
                    if (_isRecommendationVisible && mounted) {
                      setState(() => _isRecommendationVisible = false);
                    }
                  });
                ...
                // --- NẾU TRỐNG: Hiển thị màn hình Khuyến Nghị ---
                WidgetsBinding.instance.addPostFrameCallback((_) {
                  if (!_isRecommendationVisible && mounted) {
                    setState(() => _isRecommendationVisible = true);
                  }
                });
